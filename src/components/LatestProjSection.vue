<template>
  <div class="text-white mt-20" id="projects">
    <div class="px-4 xl:pl-16">
      ...
      >
        <div v-for="project in filteredProject" :key="project.id">
          <div
            class="h-52 md:h-[24rem] rounded-t-xl relative group o"
            :style="{
          ...
          >
            ...
            >
              ...
              <a :href="project.gitURL" class="h-14 w-14 border-2 relative rounded-full border-[#ADB7BE] hover:border-white group/link">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true"
                                data-slot="icon"
                                class="h-10 w-10 text-[#ADB7BE] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2  cursor-pointer group-hover/link:text-white">
          ...
          <div class="text-white rounded-b-xl mt-3 bg-[#111a3e] shadow-lg border border-[#1f1641] py-6 px-4">
             ...
             <div class="flex flex-wrap p-2.5">
                 <div style="box-shadow: 0 4px 30px rgba(0, 0, 0, 0.1); border: 1px solid #111827;backdrop-filter: blur(9px);-webkit-backdrop-filter: blur(9px);" v-for="technology in project.technologies" :key="technology" class="text-center ml-1 mt-1 rounded-3xl bg-[#111827]">
...
const myProject = ref(Projects);

const selectedCategory = ref("all");

const filteredProject = computed(() => {
  if (selectedCategory.value === "all"){
    return myProject.value;
  }
  return myProject.value.filter(project =>
