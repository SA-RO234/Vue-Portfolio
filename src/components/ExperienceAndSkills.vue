<template>
  <section class="text-white mt-18" id="skills">
    ...
    <div
      class="md:grid md:grid-cols-2 gap-8  py-8 px-4 xl:gap-16 xl:px-16 sm:py-16"
    >
      <div class="mt-4 md:mt-0 text-left flex flex-col z-10 h-full w-[80%]">
        ...
        <div class="mt-8" v-for="skil in Skills" :key="skil.id">
          <div class="flex items-end justify-between" data-aos="fade-right">
            <h4 class="font-semibold text-white uppercase">{{ skil.name }}</h4>
            <h3 class="text-2xl font-bold text-white">{{ skil.width }}</h3>
          </div>
          <div class="mt-2 h-1 w-full bg-[#131d30] rounded-full">
            <div
              class="h-1 rounded-full bg-primary"
              v-bind:style="`width:${skil.width}`"
            ></div>
          </div>
        </div>
      </div>
